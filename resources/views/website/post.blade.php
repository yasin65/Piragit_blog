      ...
      <div class="container">
        ...
        <div class="row align-items-stretch retro-layout">
          ...
          <div class="col-md-5 order-md-2">
            @foreach($relatedpost11 as $post)
            <a href="{{route('website.post',['slug'=>$post->slug])}}" class="hentry img-1 h-100 gradient" style="background-image: url('{{$post->iamge}}');">
              <span class="post-category text-white bg-danger">{{$post->category->title}}</span>
              <div class="text">
                <h2>{{$post->title}}</h2>
                <span>{{$post->created_at->format('M d, Y')}}</span>
              </div>
